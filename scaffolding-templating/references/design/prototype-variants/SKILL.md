...
description: Build multiple genuinely different versions of a UI piece you describe, rendered behind a visual picker so you can flip through them live and promote the one that feels right. Only runs when explicitly invoked; it does not trigger on its own. Renamed from the source repo's `prototype` to avoid colliding with this repo's own agentic-dev-workflow/prototype (a throwaway logic-or-UI validation skill, different mechanism — see that skill's own description).
