...
description: >
  ...
  text — scoped to English/Latin-script visible text only (see "Language Scope" below). Enforces timeless
  typographic correctness that LLMs consistently get wrong: proper quote marks, dashes, spacing, hierarchy,
  and layout. ENFORCEMENT MODE: When generating ANY HTML, CSS, React, JSX, or UI code containing visible
  ENGLISH text, auto-apply every rule in this skill silently — do not ask, do not explain, just produce
  correct typography. For Traditional/Simplified Chinese or other CJK visible text, do NOT auto-apply —
  fall back to normal explicit-trigger behavior (only when the user specifically asks for typography/copy-
  editing review), since these rules are Latin-script-specific and some (curly quotes, hyphenation, en/em
  dash usage, letterspacing on caps) actively misapply to CJK text. AUDIT MODE: When reviewing or improving
  existing interfaces or legacy code, flag violations and provide fixes (English text only, same scope
  carve-out applies). Trigger on: any HTML/CSS/React artifact creation, "build a landing page", "create a
  component", "design a UI", "fix the typography", "make this look professional", "review this layout", web
  design, presentation design, dashboard creation, document generation, or any task producing visible
  ENGLISH text for humans. Even if the user doesn't mention typography, apply these rules whenever
  generating English-language UI output — but never silently apply them to CJK output.
---

# UI Typography Skill

## Language Scope — English/Latin-script only

Every rule below (curly quotes, en/em dash distinctions, hyphenation, letterspacing on caps, line-length
in characters, kerning/ligature settings, accented-character handling) is specific to English and other
Latin-script typography. None of it transfers to Chinese (Traditional or Simplified) or other CJK text:

- CJK uses full-width punctuation (`「」『』、。`), not curly quotes (`&ldquo;`/`&rdquo;`) or the
  hyphen/en-dash/em-dash distinction above.
- CJK has no word-spacing or hyphenation concept — "one space after punctuation," soft hyphens (`&shy;`),
  and the 45–90-character line-length rule (an alphabet-based measure) don't apply.
- Letterspacing-on-all-caps and small-caps rules are meaningless for CJK, which has no case distinction.

**Enforcement scope, concretely**: silently auto-apply this skill's rules only to English-language visible
text in generated UI/HTML/CSS/React/JSX. For Chinese-language UI copy (or any other CJK content), do
**not** auto-apply — treat it like any other skill, triggering only on an explicit request (e.g. "check
the punctuation/排版 on this," "校對這段文案"). On a bilingual interface, apply these rules to the English
portions only and leave CJK portions untouched by default.

This scope carve-out was added 2026-08-16 after a user review flagged that blind auto-enforcement on a
primarily-Traditional-Chinese workflow risked silently mis-applying Latin-script rules to CJK text — see
`SKILL-AUDIT.md` for the full reasoning.
