...
description: "Systematic methodology for diagnosing bugs, test failures, or unexpected behavior before proposing a fix — and capturing the resulting root cause, blast radius, and remedy as a reusable diagnostic record. Use whenever encountering any bug, crash, failing test, or surprising behavior, BEFORE writing or proposing a fix; the goal is root-cause evidence and a clear record future debugging can reuse, not a guessed patch."
---
...
This category currently has a single source skill. It's a natural home for future skills that specifically produce structured incident write-ups (e.g. a postmortem-template skill, an on-call handoff skill) — add them under `references/` following the same pattern as the other consolidated skills in this workspace.
